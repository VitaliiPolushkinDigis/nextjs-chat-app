...
const PeerPage: FC<PeerProps> = () => {
  ...
  /*   useEffect(() => {
    navigator.mediaDevices
      ...
      .then((mediaStream) => {
        setPeer(initializePeer(mediaStream));
      });
  }, []); */
  ...
  //         peerRef.current = initializePeerInstance(stream);
  ...
  //   if (isConnected) {
  //     // Cleanup: close the peer connection if already connected
  //     if (peerRef.current) {
  ...
  return (
    <Box component={"section"} minHeight={"100vh"}>
      ...
      {/* <VideoCard videoRef={myVideo} />
      <VideoCard videoRef={remoteVideo} /> */}
      <Card>
